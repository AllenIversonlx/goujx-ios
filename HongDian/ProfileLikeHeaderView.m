...
@implementation ProfileLikeHeaderView
...
-(instancetype)initWithFrame:(CGRect)frame andtag:(NSString *)tag
{
    if (self = [super initWithFrame:frame]) {
        ...
        if (omCrmUserLikeProductString) {
            self.collectionbutton = [[UIButton alloc] initWithFrame:CGRectMake((Screen_Width() - 50 * 3 - 80 ) / 2, 193, 50, 37) andNumbleString:[NSString stringWithFormat:@"%@",omCrmUserLikeProductString] andNameString:@"我的收藏"];
        } else {
            self.collectionbutton = [[UIButton alloc] initWithFrame:CGRectMake((Screen_Width() - 50 * 3 - 80 ) / 2, 193, 50, 37) andNumbleString:@"0" andNameString:@"我的收藏"];
        }
        ...
        if ([tag isEqualToString:@"101"]) {
            if (omCrmCouponCountString) {
                self.daijinButton = [[UIButton alloc] initWithFrame:CGRectMake((Screen_Width() - 50 * 3 - 80 ) / 2 + 50 + 40, 193, 50, 37) andString:[NSString stringWithFormat:@"%@",omCrmCouponCountString] andNameString:@"代金券"];
            } else {
                self.daijinButton = [[UIButton alloc] initWithFrame:CGRectMake((Screen_Width() - 50 * 3 - 80 ) / 2 + 50 + 40, 193, 50, 37) andString:@"0" andNameString:@"代金券"];
            }
            ...
            NSArray *array = @[@{@"id":@"1",@"name":@"未使用"},@{@"id":@"2",@"name":@"已使用"},@{@"id":@"3",@"name":@"已过期"},@{@"id":@"4",@"name":@"全部"}];
            
            HeadScrollView *headView = [[HeadScrollView alloc] initWithFrame:CGRectMake(0, 252, Screen_Width() , 44) WithArray:array];
            headView.tapScrollButtonBlock = ^(NSDictionary *dic){
                if (self.selectTheCouponBlock) {
                    self.selectTheCouponBlock(dic);
                }
            };
            [self addSubview:headView];

        }  else if ([tag isEqualToString:@"102"]){
            if (omSaleOrderCountString) {
                self.orderButton = [[UIButton alloc] initWithFrame:CGRectMake((Screen_Width() - 50 * 3 - 80 ) / 2 +  180, 193, 50, 37) andString:[NSString stringWithFormat:@"%@",omSaleOrderCountString] andNameString:@"我的订单"];
            } else {
                self.orderButton = [[UIButton alloc] initWithFrame:CGRectMake((Screen_Width() - 50 * 3 - 80 ) / 2 +  180, 193, 50, 37) andString:@"0" andNameString:@"我的订单"];
            }
            self.orderButton.tag = 102;
            [self.orderButton addTarget:self action:@selector(changeTheType:) forControlEvents:UIControlEventTouchUpInside];
            [self addSubview:self.orderButton];
            ...
            [self.daijinButton addTarget:self action:@selector(changeTheType:) forControlEvents:UIControlEventTouchUpInside];
            [self addSubview:self.daijinButton];
            NSArray *array = @[@{@"id":@"1",@"name":@"全部"},@{@"id":@"2",@"name":@"未付款"},@{@"id":@"3",@"name":@"待发货"},@{@"id":@"4",@"name":@"已发货"},@{@"id":@"5",@"name":@"已取消"}];

            HeadScrollView *headView = [[HeadScrollView alloc] initWithFrame:CGRectMake(0, 252, Screen_Width() , 44) WithArray:array];
            headView.tapScrollButtonBlock = ^(NSDictionary *dic){
                if (self.selectThPayOrderBlock) {
                    self.selectThPayOrderBlock(dic);
                }
            };
            [self addSubview:headView];
        }
